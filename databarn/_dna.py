...
class ObDna:
    ...
    def __init__(self, cob: "Cob"):
        ...
        """
        model = cob.__class__
        if model.__dna__.dynamic:
            model = copy.copy(model)  # Create a shallow copy of the model class
        self.model = model
        for name in model.__dna__.__annotations__.keys():
            value = getattr(model, name, None)
            setattr(self, name, value) # To show up in dir() and help()
        self.cob = cob
        self.barns = []
        self.autoid = id(cob)  # Default autoid is the id of the cob object
    
    def __getattribute__(self, name):
        model_dna = super().__getattribute__('model').__dna__
        if not name.startswith("_") and name in model_dna.__annotations__.keys():
            # Ensure that the attribute is always fetched from the model's dna
            return getattr(model_dna, name)
