...
class BaseDna:
    ...
    @classmethod_only
    def _embed_grain(klass, label: str, grain: type[BaseGrain]) -> None:
    ...
    def __init__(self, cob: "Cob") -> None:  # type: ignore
        ...
        for grain_class in grain_classes:
            grainob = grain_class(cob)
            self._embed_grainob(grain_class.label, grainob)

    def _embed_grainob(self, label: str, grainob: BaseGrain) -> None:
        """Embed a grain instance under `label` in the Cob instance."""
        if label in self.labels:
            raise CobConsistencyError(fo(f"""
                The Grain '{label}' has already been
                set up in this {self}."""))
        self.label_grain_map[label] = grainob  # type: ignore
    ...
    def add_grain(self, label: str,
        ...
        if self.design != "dynamic":
            self.__class__._embed_grain(label, grain)
            ...
            this_grainob = grain(self.cob)
            self._embed_grainob(label, this_grainob)
